...
save_dir <- "../results/simulation_linear_elasticnet"
...
alpha_grid   <- c(0, 0.25, 0.50, 0.75, 1)      # l1–penalties for glmnet
...
get_E <- function(Xmat, Yvec, stat_fun, l1_penalty = 1) {
  if (identical(stat_fun, stat.glmnet_coefdiff)) {
    ekn(Xmat, Yvec, M, alpha / 2, mu, Sigma, diags,
        family = "gaussian", offset = 1, l1_penalty = l1_penalty)$E
  } else {
    warning("No paralelization. Ensure glmnet is used")
    sm <- kn_stat(Xmat, Yvec, M, mu, Sigma, diags,
                  stat_method = stat_fun, family = "gaussian")
...
## Main loop
seedB <- 1                                        # Useful if you run line-by-line
for (seedB in seq_len(nrep)) {
  
  message(sprintf("Repetition %d / %d", seedB, nrep))
  set.seed(100 + (seedA - 1) * nrep + seedB)      # reproducible
  
  Xk <- create.gaussian(X, mu, Sigma, diag_s = diags) # Used for single vanilla knockoff
  
  E_train_list <- list()
  E_test_list  <- list()
  power_train  <- numeric(length(alpha_grid))
  
  ## Individual elastic net models
  for (m in seq_along(alpha_grid)) {
    
    a     <- alpha_grid[m]
    mname <- model_names[m]
    
    ## Vanilla knockoff filter
    W   <- stat.glmnet_coefdiff(X, Xk, Y, family = "gaussian", alpha = a, cores=6, nlambda = 50)
    ...
    ## Derandomised knckoffs (e-values) using training data
    E_train <- get_E(X, Y,
  ...
  # generate all e-values again on test data
  set.seed(2000 + seedB)
  ...
  # Performance-weighted (power) ensemble
