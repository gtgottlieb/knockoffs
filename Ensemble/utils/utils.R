...
ekn <- function(X, Y, M, gamma, 
                mu, Sigma, diags, 
                family = "gaussian", offset = 1, l1_penalty = 1){
  ...
  for(m in 1:M){
    Xk <- create.gaussian(X, mu, Sigma, diag_s = diags)
    W <- stat.glmnet_coefdiff(X, Xk, Y, family = family, alpha = l1_penalty, cores=6, nlambda = 500)
    tau <- stop_early(W, gamma, offset) 
...
weighted_ekn <- function(X, Y, M, alpha, gamma, weight, 
  ...
  for(m in 1:M){
    Xk <- create.gaussian(X, mu, Sigma, diag_s = diags)
    W <- stat.glmnet_coefdiff(X, Xk, Y, family = family, cores=6, nlambda = 500)        
    tau <- stop_early(W, gamma, offset)
