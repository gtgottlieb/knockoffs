...
n <- 1000
p <- 800
k <- 80             # Number of active X-variables
alpha <- 0.1
rho <- 0.5
M <- 50
mu <- rep(0,p)
...
y.gen <- function(X, beta_lin){
  g <- X %*% beta_lin +               # beta carries amp (& nonzero vector)
    rowSums((X[, nonzero]^2) / 2) +   # quadratic term (same nonzero vector)
    rnorm(n)
  drop(g)
}
...
stat.xgb_gain_diff <- function(X, Xk, y,
  ...
  imp  <- xgb.importance(feature_names = colnames(Xall),  
                         model = bst)
